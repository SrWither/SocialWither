...
namespace Api {
void AuthController::getUser(Request &req, Callback &&callback) {
  ...
  userApiReq->setPath(fmt::format("/users/{}", userId));
  userApiReq->setContentTypeCode(drogon::CT_APPLICATION_JSON);
  userApiReq->setMethod(drogon::Get);

  userApi->sendRequest(userApiReq, [callback = std::move(callback)](
